 ...
 *
 * Fuzz target for oidc_util_json_decode_object(): decoding of
...
#include "util/util.h" /* oidc_util_json_decode_object */
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
	...
	json_t *json = NULL;
	if ((oidc_util_json_decode_object(&r, s, &json) == TRUE) && (json != NULL))
		json_decref(json); /* jansson value is refcounted, not pooled */
